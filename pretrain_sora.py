...
from mindspeed_mm.data.data_utils.constants import VIDEO, PROMPT_IDS, PROMPT_MASK
...
def get_batch_on_this_tp_rank(data_iterator):
    ...
    batch = {VIDEO: video, PROMPT_IDS: prompt_ids, PROMPT_MASK: prompt_mask}
    return batch
...
def get_batch(data_iterator):
    ...
    if mpu.is_pipeline_first_stage():
        ...
        return batch[VIDEO], batch[PROMPT_IDS], batch[PROMPT_MASK]
    else:
...
def forward_step(data_iterator, model):
    """Forward step."""
    video, prompt_ids, prompt_mask = get_batch(data_iterator)
    output_tensor_list = model(video, prompt_ids, prompt_mask=prompt_mask)
