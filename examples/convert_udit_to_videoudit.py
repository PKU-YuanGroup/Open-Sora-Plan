...
def conv2d_to_conv3d(name, conv2d_weights, k_size):
    ...
    conv3d_weights[:,:,0,:,:] = conv2d_weights / 4
    conv3d_weights[:,:,1,:,:] = conv2d_weights / 1
    conv3d_weights[:,:,2,:,:] = conv2d_weights / 4
    return conv3d_weights

path = "/storage/ongoing/new/checkpoints/bs16_4node_480p_lr1e-4_snr5_noioff0.02_ema_rope_uditultra22_qknorm_ds22_mt5xxl_mjencn_czhan_humanimg/checkpoint-73000/model_ema/diffusion_pytorch_model.safetensors"
ckpt = safe_load(path, device="cpu")
new_ckpt = {}
k_size = 3
patch_size_t = 1
for k, v in ckpt.items():
    ...
    if 'attn1.downsampler.layer.weight' in k:
        new_v = conv2d_to_conv3d(k, v, k_size)  # 768, 4, 3, 3 -> 768, 4, 3, 3, 3
    # elif 'body.0.weight' in k and 'down' in k:
    ...
    #     new_v = conv2d_to_conv3d(k, v, k_size)  # 6144, 3072, 3, 3 -> 6144, 3072, 3, 3, 3
        ...
        new_v = v
    new_ckpt[k] = new_v
torch.save(new_ckpt, f'/storage/ongoing/new/image2video_weight/480p_73000_ema_ds_k{k_size}_p{patch_size_t}_repeat_lowsize4.pt')