...
def prepare_parallel_data(hidden_states, encoder_hidden_states, attention_mask, encoder_attention_mask, use_image_num):
    ...
    if use_image_num == 0:
        ...
        video_encoder_states, image_encoder_states = encoder_hidden_states[:, :-use_image_num], encoder_hidden_states[:, -use_image_num:]
        video_attention_mask, image_attention_mask = attention_mask[:, :-use_image_num], attention_mask[:, -use_image_num:]
        video_encoder_attention_mask, image_encoder_attention_mask = encoder_attention_mask[:, :-use_image_num], encoder_attention_mask[:, -use_image_num:]
        ...
        video_states, video_encoder_states, video_attention_mask, video_encoder_attention_mask = all_to_all(video_states,
        ...
        image_encoder_attention_mask = F.pad(image_encoder_attention_mask, (0, 0, 0, padding_needed), mode='constant', value=0)
        image_states, image_encoder_states, image_attention_mask, image_encoder_attention_mask = all_to_all(image_states,
