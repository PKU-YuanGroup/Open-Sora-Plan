...
class LandscopeFeature(data.Dataset):
    def __init__(self, args, transform, temporal_sample):
        ...
        self.data_all = [i for i in tqdm(data_all) if self.num_frames * self.sample_rate < np.load(i).shape[1]]
...
class SkyFeature(data.Dataset):
    def __init__(self, args, transform, temporal_sample=None):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            start_frame_ind, end_frame_ind = self.temporal_sample(total_frames)
            assert end_frame_ind - start_frame_ind >= self.num_frames
            frame_indice = np.linspace(start_frame_ind, end_frame_ind - 1, num=self.num_frames,
                                       dtype=int)  # start, stop, num=50
    ...
    def load_video_frames(self, dataroot):
        ...
        for _, meta in enumerate(frame_list):
            ...
            if len(frames) > max(0,
                                 self.target_video_len * self.frame_interval):  # need all > (16 * frame-interval) videos
