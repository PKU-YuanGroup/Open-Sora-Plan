...
from opensora.models.diffusion.diffusion import create_diffusion_navit as create_diffusion
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, (x_list, input_ids, cond_mask) in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                # B [C 1+T+num_images H W]
                x_list = list(map(lambda x: x.to(accelerator.device, dtype=weight_dtype), x_list))
                attn_mask = None
                ...
                with torch.no_grad():
                    ...
                    if args.use_image_num == 0:
                        for x in x_list:
                            x = ae.encode(x.unsqueeze(0))  # 1 C T H W
                            latent_list.append(x.squeeze(0))
                    else:
                        for x in x_list:
                            x = x.unsqueeze(0)  # 1 C T H W
                            videos, images = x[:, :, :-args.use_image_num], x[:, :, -args.use_image_num:]
                            videos = ae.encode(videos)  # 1 C T H W
                ...
                t = torch.randint(0, diffusion.num_timesteps, (len(x_list),), device=accelerator.device)
                loss_dict = diffusion.training_losses(model, latent_list, t, model_kwargs)
                loss = loss_dict["loss"]
