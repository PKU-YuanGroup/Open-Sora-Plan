...
def hacked_model(model, model_type=ModelType.VIP_ONLY, model_cls=OpenSoraT2V):
    model._operate_on_patched_inputs = operate_on_patched_inputs(model_type).__get__(model, model_cls)
    model.forward = forward_for_model(model_type).__get__(model, model_cls)
    model.custom_load_state_dict = inpaint_model_load_state_dict(model_type).__get__(model, model_cls)
...
def inpaint_model_load_state_dict(model_type=ModelType.INPAINT_ONLY):
    def hacked_func(self, pretrained_model_path, load_mask=False):
        return hacked_load_state_dict(self, pretrained_model_path, load_mask, model_type)
    return hacked_func


def hacked_load_state_dict(self, pretrained_model_path, load_mask=False, model_type=ModelType.INPAINT_ONLY):
...
def operate_on_patched_inputs(model_type=ModelType.VIP_ONLY):
    def hacked_func(
        ...
        frame, 
        use_image_num, 
        vip_hidden_states=None,
    ):
        return hacked_operate_on_patched_inputs(
            self,
            ...
            vip_hidden_states=vip_hidden_states,
            model_type=model_type,
        )
    return hacked_func

def forward_for_model(model_type=ModelType.VIP_ONLY):
    def hacked_func(
        self,
...
def hacked_operate_on_patched_inputs(self, hidden_states, encoder_hidden_states, timestep, added_cond_kwargs, batch_size, frame, use_image_num, vip_hidden_states=None, model_type=ModelType.VIP_ONLY, ):

    if model_type != ModelType.VIP_ONLY:
        # inpaint
        assert hidden_states.shape[1] == 3 * self.config.in_channels, f"hidden_states.shape[1] {hidden_states.shape[1]} != 3 * self.config.in_channels {3 * self.config.in_channels}"
        in_channels = self.config.in_channels
        hidden_states, hidden_states_masked_vid, hidden_states_mask = hidden_states[:, :in_channels], hidden_states[:, in_channels: 2 * in_channels], hidden_states[:, 2 * in_channels: 3 * in_channels]
    ...
    if self.caption_projection is not None:
        ...
        if model_type != ModelType.INPAINT_ONLY:
            ...
            encoder_hidden_states = torch.cat([encoder_hidden_states, vip_hidden_states], dim=2)  # # B 1 N D -> B 1 N+num_vip_tokens D
...
def hacked_forward(
    ...
    if model_type != ModelType.INPAINT_ONLY:
        ...
        timestep_vid, timestep_img, embedded_timestep_vid, embedded_timestep_img = self._operate_on_patched_inputs(
            hidden_states, encoder_hidden_states, timestep, added_cond_kwargs, batch_size, frame, use_image_num, vip_hidden_states=vip_hidden_states,
        )
    else:
        hidden_states_vid, hidden_states_img, encoder_hidden_states_vid, encoder_hidden_states_img, \
        timestep_vid, timestep_img, embedded_timestep_vid, embedded_timestep_img = self._operate_on_patched_inputs(
