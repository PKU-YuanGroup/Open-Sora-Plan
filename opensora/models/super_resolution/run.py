...
def save_frame(frame_count, frame, temp_LR_folder_path):
    try:
        # print(f"Saving frame {frame_count}")
        frame_path = os.path.join(temp_LR_folder_path, f"frame_{frame_count}.png")
        # print(frame_path,frame_count)
...
def video_sr(args):
    ...
    temp_LR_folder_path = os.path.join(args.output_dir, f'temp_LR')
    temp_result_folder_path = os.path.join(args.output_dir, f'temp_results/Single')
    create_temp_folder(temp_LR_folder_path)
    create_temp_folder(temp_result_folder_path)
    ...
    with ThreadPoolExecutor(max_workers = args.mul_numwork) as executor:
        for frame_count, frame in frames_to_process:
            executor.submit(save_frame, frame_count, frame, temp_LR_folder_path)
    
    t2 = time.time()
    print('multi threads to save video to frame: ',t2 - t1,'s')
    ...
    frame_files = sorted(os.listdir(temp_result_folder_path), key=extract_number)
    video_frames = [imageio.imread(os.path.join(temp_result_folder_path, frame_file)) for frame_file in frame_files]
    ...
    cap.release()
    # delete_temp_folder(temp_LR_folder_path)
    # delete_temp_folder(os.path.dirname(temp_result_folder_path))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_path", type=str, default = "/remote-home/lzy/RGT/experiments/pretrained_models/RGT_x4.pth",help ='# The path to place the ckpt')
    
    parser.add_argument("--root_path", type=str, default = "/remote-home/lzy/RGT")
    parser.add_argument("--input_dir", type=str, default= "/remote-home/lzy/RGT/datasets/video/test__5_512.mp4")
    ...
    parser.add_argument("--use_chop", type= bool, default = False,  help ='use_chop: True  # True to save memory, if img too large')
