...
class NaViT(nn.Module):
    ...
    """
        ...
        assert divisible_by(image_height, patch_size) and divisible_by(image_width,
                                                                       patch_size), 'Image dimensions must be divisible by the patch size.'
    ...
    def forward(
        ...
        for images in batched_images:
            ...
            for image_id, image in enumerate(images):
                ...
                image_dims = image.shape[-2:]
                assert all([divisible_by(dim, p) for dim in
                            image_dims]), f'height and width {image_dims} of images must be divisible by patch size {p}'
