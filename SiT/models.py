...
class FinalLayer(nn.Module):
    ...
    """
    def __init__(self, hidden_size, temporal_size, patch_size, out_channels):
        super().__init__()
        self.norm_final = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.linear = nn.Linear(hidden_size, temporal_size * patch_size * patch_size * out_channels, bias=True)
        self.adaLN_modulation = nn.Sequential(
            nn.SiLU(),
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        self.temporal_size = temporal_size = 1
        self.num_heads = num_heads
        ...
        self.final_layer = FinalLayer(hidden_size, temporal_size, patch_size, self.out_channels)
    ...
    def unpatchify(self, x):
        """
        x: (B, N, temporal_size*patch_size**2 * C)
        imgs: (B, T, H, W, C)
        """
        c = self.out_channels
        p = self.x_embedder.patch_size[0]
        o = self.temporal_size
    ...
    def forward(self, x, t, y, attention_mask):
        ...
        self.t = num_tubes_length = T // self.temporal_size  # 1
        ...
        x = self.x_embedder(x) + pos_embed  # (B, N, D), where N = H * W / patch_size ** 2
        ...
        for block in self.blocks:
            x = block(x, c, attention_mask)                      # (B, N, D)
